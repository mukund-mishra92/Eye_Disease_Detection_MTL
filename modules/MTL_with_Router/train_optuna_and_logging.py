...
def objective(trial: Trial):
    ...
    loss_weight_cls = trial.suggest_float("loss_weight_cls", 0.3, 0.7)
    loss_weight_seg = 1.0 - loss_weight_cls
    ...
    for epoch in range(10):
        ...
        total_cls_loss, total_seg_loss = 0.0, 0.0
        ...
        avg_seg_loss = total_seg_loss / steps
        combined_train_loss = loss_weight_cls * avg_cls_loss + loss_weight_seg * avg_seg_loss
        ...
        logging.info(f"Epoch {epoch+1} | Cls Loss: {avg_cls_loss:.4f} | Seg Loss: {avg_seg_loss:.4f} | Combined: {combined_train_loss:.4f} | Val Loss: {val_loss:.4f}")

        if trial.should_prune():
            raise optuna.exceptions.TrialPruned()

    with open("optuna_training_history.json", "w") as f:
